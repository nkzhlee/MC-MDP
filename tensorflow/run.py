...
def parse_args():
    ...
    train_settings.add_argument('--min_cnt', type=int, default=2,
                                help='tokens with frequency less than min_cnt is filtered')
    ...
    model_settings.add_argument('--max_p_len', type=int, default=50,
                                help='max sentence of a passage')
    model_settings.add_argument('--max_s_len', type=int, default=200,
                                help='max word lenth of a sentence')
    model_settings.add_argument('--max_q_len', type=int, default=60,
                                help='max word length of question')
    model_settings.add_argument('--max_a_len', type=int, default=50,
    ...
    path_settings.add_argument('--train_files', nargs='+',
                               default=['../data/demo/100_train.json'],
                               help='list of files that contain the preprocessed train data')
...
def prepare(args):
    ...
    mrc_data = MRCDataset(args.max_p_num, args.max_p_len, args.max_q_len, args.max_s_len,
                          args.train_files, args.dev_files, args.test_files)
    logger.info('Building dateset success')
    vocab = Vocab(lower=True)
    for word in mrc_data.word_iter('train'):
        vocab.add(word)
...
def train(args):
    ...
    brc_data = MRCDataset(args.max_p_num, args.max_p_len, args.max_q_len, args.max_s_len,
...
def evaluate(args):
    ...
    brc_data = MRCDataset(args.max_p_num, args.max_p_len, args.max_q_len, args.max_s_len, dev_files=args.dev_files)
    logger.info('Converting text into ids...')
    brc_data.convert_to_ids(vocab)
